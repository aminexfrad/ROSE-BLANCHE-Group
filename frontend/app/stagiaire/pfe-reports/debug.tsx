...
export default function PFEReportsDebug() {
  ...
  const testAPI = async () => {
    ...
    try {
      ...
      const backendResponse = await fetch('http://localhost:8000http://localhost:8000/api/pfe-reports/', {
        headers: {
      ...
      const proxyResponse = await fetch('http://localhost:8000/api/pfe-reports/', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      const noAuthResponse = await fetch('http://localhost:8000http://localhost:8000/api/pfe-reports/')
  ...
  return (
    <div className="p-6">
      <Card>
        <CardHeader>
          <CardTitle>Debug PFE Reports API</CardTitle>
        </CardHeader>
        <CardContent>
